 ...
 * @return {Object}
 */
users.dataValidation = function ( user_data ) {
   let errors = {};
...
users.post = function( data, callback ) {
   ...
   const data_validation = users.dataValidation( user_data );

   // Validate received data
   if ( !Object.keys( user_data ).length ) {
      return callback( 400, { success: false, message: 'No allowed properties were passed' } );
   }

   if ( Object.keys( data_validation ).length > 0 ) {
      return callback( 400, { success: false, message: 'No valid properties were provided', error: data_validation } );
   }
   ...
   if ( !user_data.password ) {
      return callback( 500, { success: false, message: users.errors[ users.errors.CODES.FAIL_PASSWORD_HASH ] } );
   }
   ...
   db.read( 'users', file_name )
      ...
      .then( () => callback( 500, { success: false, message: 'User with this email are already exist' } ) )
      // Can't open this file, which for opening is an error, but for use means that such user do not exists and we can proceed.
      .catch( () => {
         db.create( 'users', file_name, user_data )
         .then( () => callback( 200, { success: true } ) )
         .catch( ( error_code, message, error ) => callback( 500, { success: false, message: users.errors[ error_code ], error } ) );
      });
};
...
users.get = function( data, callback ) {
   const user_data = helpers.validateData( { email: data.props.email || data.query.email }, users.properties );
   const file_name = helpers.mailToFileName( user_data.email );

   if ( !helpers.isEmail( user_data.email ) ) {
      return callback( 400, { success: false, message: 'No valid email is provided' } );
   }

   verifyToken( data.headers, user_data.email )
...
users.put = function( data, callback ) {
   ...
   if ( !Object.keys( user_data ).length  ) {
      return callback( 400, { success: false, message: 'Missing fields to update' } );
   }
   ...
   verifyToken( data.headers, user_email )
      .then( () => db.read( 'users', file_name ) )
...
users.delete = function( data, callback ) {
   ...
   if ( !helpers.isEmail( user_data.email ) ) {
      return callback( 400, { success: false, message: 'Missing required field - email'} );
   }

   verifyToken( data.headers, user_data.email )
      .then( () => db.read( 'users', file_name ) )
      .then( () => db.delete( 'users', file_name ) )
      .then( () => callback( 200, { success: true } ) )
      .catch( ( error_code, message, error ) => callback( 500, { success: false, message : users.errors[ error_code ], error } ) );
};
